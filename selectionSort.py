...
# Selection sort algorithm without acceleration
def selection_sort(arr):
    ...
    for i in range(n - 1):
        ...
        if min_idx != i:
            ...
            frequency2 = base_frequency + (arr[min_idx] / max(arr)) * octave_range * base_frequency
            ...
            update_columns(arr, [x * (COLUMN_WIDTH + COLUMN_GAP) + 50 for x in range(len(arr))], max(arr), highlight_indices=[i, min_idx])
...
time.sleep(1)
# Run selection sort and visualize
selection_sort(arr)
