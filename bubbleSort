...
# Bubble sort algorithm without acceleration
def bubble_sort(arr):
    n = len(arr)
    ...
    for i in range(n):
        ...
        for j in range(0, n-i-1):
            if arr[j] > arr[j+1]:
                ...
                frequency1 = base_frequency + (arr[j] / max(arr)) * octave_range * base_frequency
                frequency2 = base_frequency + (arr[j+1] / max(arr)) * octave_range * base_frequency
